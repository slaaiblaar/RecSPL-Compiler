...
// updates classes to be consistent with the classes generated by the tester
// ie. nonterminal classes == word,
// terminal classes =
//      KEYWORD,
//      LITERAL (vor CONST values) or
//      VIF/FID for function and variable names
void change_class(std::shared_ptr<node> n)
{
    if (n->CLASS == "N_Token" || n->CLASS == "T_Token")
    {
        n->CLASS = "LITERAL";
    }
    else if (n->CLASS == "F_Token")
    {
        n->CLASS = "FID";
    }
    else if (n->CLASS == "V_Token")
    {
        n->CLASS = "VID";
    }
    else if (n->CLASS == "NONTERMINAL")
    {
        n->CLASS = n->WORD;
...
std::shared_ptr<node> Parser::parse(std::string dest_name, std::string token_file)
{
    ...
    if (accept)
    {
        ...
        change_class(parse_stack.top().second);
