...
std::string print_code(std::shared_ptr<node> n, int depth, std::ofstream &code_file)
{
    ...
    {
        product = print_code(n->get_child(0), depth, code_file);
    }
    ...
    {
        std::string main_str = print_code(n->get_child(0), depth, code_file);
        std::string globvars = "";
        std::string algo = "";
        std::string functions = "";
        int index = 1;

        if (n->get_child(1)->CLASS == "GLOBVARS")
        {
            globvars = print_code(n->get_child(1), depth, code_file);
            algo = print_code(n->get_child(2), depth, code_file);
            index = 2;
        }
        else
        {
            algo = print_code(n->get_child(1), depth, code_file);
        }

        if (c.size() == index + 2)
        {
            functions = print_code(c[index + 1], depth, code_file);
        }
    ...
    else if (n->CLASS == "INSTRUC")
    {
        product = "{}{}{}\n";
        std::string indentation = std::string(depth * 4, ' ');
        std::string command = print_code(n->get_child(0), depth, code_file);
        std::string semicolon = print_code(n->get_child(1), depth, code_file);
        std::string instruc;
        int num_insertions = 3;
        if (c.size() > 2)
        {
            ++num_insertions;
            instruc = print_code(n->get_child(2), depth, code_file);
        }
        if (num_insertions == 3)
        {
    ...
    {
        if (n->get_child(0)->CLASS == "FNAME")
        {
            std::string fname = print_code(n->get_child(0), depth, code_file);
            std::string open = print_code(n->get_child(1), depth, code_file);
            std::string atomic1 = print_code(n->get_child(2), depth, code_file);
            std::string comma1 = print_code(n->get_child(3), depth, code_file);
            std::string atomic2 = print_code(n->get_child(4), depth, code_file);
            std::string comma2 = print_code(n->get_child(5), depth, code_file);
            std::string atomic3 = print_code(n->get_child(6), depth, code_file);
            std::string close = print_code(n->get_child(7), depth, code_file);
            product = "{}{}{}{}{}{}{}{}";
            product = fmt::format("{}{}{}{}{}{}{}{}", fname, open, atomic1, comma1, atomic2, comma2, atomic3, close);
        }
        else if (n->get_child(0)->WORD == "return" || n->get_child(0)->WORD == "print")
        {
            std::string keyword = print_code(n->get_child(0), depth, code_file);
            std::string atomic = print_code(n->get_child(1), depth, code_file);
            product = "{}{}";
            product = fmt::format("{}{}", keyword, atomic);
        }
        else
        {
            std::string symbol = print_code(n->get_child(0), depth, code_file);
            product = fmt::format("{}", symbol);
        }
    }
    else if (n->CLASS == "FUNCTIONS")
    {
        std::string decl = print_code(n->get_child(0), depth, code_file);
        if (c.size() > 1)
        {
            std::string functions = print_code(n->get_child(1), depth, code_file);
            product = fmt::format("{}\n{}", decl, functions);
        }
        else
        {
    ...
    {
        product = print_code(n->get_child(0), depth + 1, code_file);
    }
    else if (n->CLASS == "DECL")
    {
        std::string head = print_code(n->get_child(0), depth, code_file);
        std::string body = print_code(n->get_child(1), depth, code_file);
    ...
    {
        std::string vtyp = print_code(n->get_child(0), depth, code_file);
        std::string vname = print_code(n->get_child(1), depth, code_file);
        if (c.size() > 2)
        {
            std::string globvars = print_code(n->get_child(2), depth, code_file);
    ...
    else if (n->CLASS == "OP")
    {
        if (c.size() > 4)
        {
            std::string binop = print_code(n->get_child(0), depth, code_file);
            std::string open = print_code(n->get_child(1), depth, code_file);
            std::string arg1 = print_code(n->get_child(2), depth, code_file);
            std::string comma = print_code(n->get_child(3), depth, code_file);
            std::string arg2 = print_code(n->get_child(4), depth, code_file);
            std::string close = print_code(n->get_child(5), depth, code_file);
            product = fmt::format("{}{}{}{}{}{}", binop, open, arg1, comma, arg2, close);
        }
        else
        {
            std::string unop = print_code(n->get_child(0), depth, code_file);
            std::string open = print_code(n->get_child(1), depth, code_file);
            std::string arg = print_code(n->get_child(2), depth, code_file);
            std::string close = print_code(n->get_child(3), depth, code_file);
            product = fmt::format("{}{}{}{}", unop, open, arg, close);
        }
    }
    else if (n->CLASS == "ASSIGN")
    {
        std::string vname = print_code(n->get_child(0), depth, code_file);
        if (c.size() == 2)
        {
            std::string input = print_code(n->get_child(1), depth, code_file);
            product = fmt::format("{}{}", vname, input);
        }
        else if (c.size() == 3)
        {
            std::string assignment = print_code(n->get_child(1), depth, code_file);
            std::string term = print_code(n->get_child(2), depth, code_file);
            product = fmt::format("{}{}{}", vname, assignment, term);
        }
        else // 10
        {
            std::string assignment = print_code(n->get_child(1), depth, code_file);
            std::string fname = print_code(n->get_child(2), depth, code_file);
            std::string open = print_code(n->get_child(3), depth, code_file);
            std::string atomic1 = print_code(n->get_child(4), depth, code_file);
            std::string comma1 = print_code(n->get_child(5), depth, code_file);
            std::string atomic2 = print_code(n->get_child(6), depth, code_file);
            std::string comma2 = print_code(n->get_child(7), depth, code_file);
            std::string atomic3 = print_code(n->get_child(8), depth, code_file);
            std::string close = print_code(n->get_child(9), depth, code_file);
    ...
    else if (n->CLASS == "COMPOSIT")
    {
        if (c.size() > 4)
        {
            std::string op = print_code(n->get_child(0), depth, code_file);
            std::string open = print_code(n->get_child(1), depth, code_file);
            std::string simple1 = print_code(n->get_child(2), depth, code_file);
            std::string comma = print_code(n->get_child(3), depth, code_file);
            std::string simple2 = print_code(n->get_child(4), depth, code_file);
            std::string close = print_code(n->get_child(5), depth, code_file);
        ...
        else
        {
            std::string op = print_code(n->get_child(0), depth, code_file);
            std::string open = print_code(n->get_child(1), depth, code_file);
            std::string simple = print_code(n->get_child(2), depth, code_file);
            std::string close = print_code(n->get_child(3), depth, code_file);
    ...
    {
        std::string vtyp1 = print_code(n->get_child(0), depth, code_file);
        std::string vname1 = print_code(n->get_child(1), depth, code_file);
        std::string comma1 = print_code(n->get_child(2), depth, code_file);
        std::string vtyp2 = print_code(n->get_child(3), depth, code_file);
        std::string vname2 = print_code(n->get_child(4), depth, code_file);
        std::string comma2 = print_code(n->get_child(5), depth, code_file);
        std::string vtyp3 = print_code(n->get_child(6), depth, code_file);
        std::string vname3 = print_code(n->get_child(7), depth, code_file);
        std::string comma3 = print_code(n->get_child(8), depth, code_file);
        product = fmt::format("{}{}{}{}{}{}{}{}{}", vtyp1, vname1, comma1, vtyp2, vname2, comma2, vtyp3, vname3, comma3);
    }
    else if (n->CLASS == "HEADER")
    {
        std::string str = print_code(n->get_child(0), depth, code_file);
        product = str;
        str = print_code(n->get_child(1), depth, code_file);
        product = fmt::format("{}{}", product, str);
        str = print_code(n->get_child(2), depth, code_file);
        product = fmt::format("{}{}", product, str);
        str = print_code(n->get_child(3), depth, code_file);
        product = fmt::format("{}{}", product, str);
        str = print_code(n->get_child(4), depth, code_file);
        product = fmt::format("{}{}", product, str);
        str = print_code(n->get_child(5), depth, code_file);
        product = fmt::format("{}{}", product, str);
        str = print_code(n->get_child(6), depth, code_file);
        product = fmt::format("{}{}", product, str);
        str = print_code(n->get_child(7), depth, code_file);
        product = fmt::format("{}{}", product, str);
        str = print_code(n->get_child(8), depth, code_file);
        product = fmt::format("{}{}", product, str);
    }
    // }
    ...
    {
        std::string indentation = std::string((depth) * 4, ' ');
        std::string prolog = print_code(n->get_child(0), depth, code_file);
        std::string locvars = print_code(n->get_child(1), depth + 1, code_file);
        std::string algo = print_code(n->get_child(2), depth + 1, code_file);
        std::string epilog = print_code(n->get_child(3), depth, code_file);
        if (c.size() == 5)
        {
            std::string end = print_code(n->get_child(4), depth, code_file);
            product = fmt::format("{}{}\n{}    {}\n{}\n{}{}\n{}{}", indentation, prolog, indentation, locvars, algo, indentation, epilog, indentation, end);
        }
        else
        {
            std::string subfuncs = print_code(n->get_child(4), depth, code_file);
            std::string end = print_code(n->get_child(5), depth, code_file);
            product = fmt::format("{}{}\n{}    {}\n{}\n{}{}\n{}\n{}{}", indentation, prolog, indentation, locvars, algo, indentation, epilog, subfuncs, indentation, end);
        }
    }
    else if (n->CLASS == "ALGO")
    {
        std::string begin = print_code(n->get_child(0), depth, code_file);
        std::string end = print_code(c[c.size() - 1], depth, code_file);
        std::string indentation = std::string(depth * 4, ' ');
        if (c.size() > 2)
        {
            std::string instruc = print_code(n->get_child(1), depth + 1, code_file);
    ...
    {
        std::string _if = print_code(n->get_child(0), depth, code_file);  // if
        std::string cond = print_code(n->get_child(1), depth, code_file); // COND
        code_file << std::string(indentation, depth);
        std::string _then = print_code(n->get_child(2), depth, code_file);      // then
        std::string t_algo = print_code(n->get_child(3), depth + 1, code_file); // ALGO
        std::string _else = print_code(n->get_child(4), depth, code_file);      // else
        std::string e_algo = print_code(n->get_child(5), depth + 1, code_file); // ALGO
        std::string indentation = std::string(depth * 4, ' ');
...
std::string node::printnode(int depth, std::string called_from)
{
    ...
    std::string product;
    std::string head = fmt::format("{}<{} word=\"{}\" uid=\"{}\" class=\"{}\" printed=\"{}\" node_id=\"{}\">\n", root_indent, this->NAME, this->WORD, this->UID, this->CLASS, this->was_printed, std::to_string(this->node_id));
    ...
    product = fmt::format("{}{}{}{}{}</{}>\n", head, str_ftable, str_vtable, chidren_str, root_indent, this->WORD);
    return product;
}
